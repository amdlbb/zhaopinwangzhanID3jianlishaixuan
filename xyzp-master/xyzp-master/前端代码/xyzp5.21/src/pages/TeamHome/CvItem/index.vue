                ...
                <el-table :data="tableData" border style="width: 100%">
                    ...
                    <el-table-column label="是否推荐" width="100">
                        <template slot-scope="scope">
                            <span v-show="scope.row.predictResult === true" class="cvItem-recommend">推荐</span>
                            <span v-show="scope.row.predictResult === false" class="cvItem-notrecommend">不推荐</span>
                            <span v-show="scope.row.predictResult === undefined || scope.row.predictResult === null" class="cvItem-unknown">未知</span>
                        </template>
                    </el-table-column>
        ...
        async search() {
            ...
            if (res.code == 200) {
              this.tableData = res.data.data.sort((a, b) => {
                if (a.predictResult === true && b.predictResult !== true) return -1;
                if (a.predictResult !== true && b.predictResult === true) return 1;
                return 0;
              });
